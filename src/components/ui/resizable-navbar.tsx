...
export const Navbar = ({ children, className }: NavbarProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn("sticky inset-x-0 top-40 z-40 w-full", className)}
    >
    ...
    >
      {items.map((item, idx) => (
        <Link
          ...
          onClick={onItemClick}
          className="cursor-target relative px-4 py-2 text-neutral-600 dark:text-neutral-300"
          key={`link-${idx}`}
          to={item.link}
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full rounded-full bg-black/20 dark:bg-white/20"
            />
          )}
